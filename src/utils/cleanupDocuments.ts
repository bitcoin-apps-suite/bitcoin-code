...
export const cleanupEmptyDocuments = () => {
  const documentsKey = 'bitcoin_app_documents';
...
export const clearAllDocuments = () => {
  ...
  if (confirmed) {
    localStorage.removeItem('bitcoin_app_documents');
    localStorage.removeItem('bitcoin_app_current_doc');
...
export const deleteDocumentById = (docId: string) => {
  ...
  try {
    ...
    if (filteredDocs.length < documents.length) {
      ...
      const currentDoc = localStorage.getItem('bitcoin_app_current_doc');
      if (currentDoc === docId) {
        localStorage.removeItem('bitcoin_app_current_doc');
      }
