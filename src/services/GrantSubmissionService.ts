...
export class GrantSubmissionService {
  ...
  /**
   * Award $BAPP tokens to a submission (platform curation signal)
   */
  async awardBAppTokens(submissionId: string, awardAmount: number, reviewNotes?: string): Promise<GrantSubmission> {
    ...
    return `
      ...
      <body>
        ...
        <div class="section">
          ...
          <p><span class="label">Platform:</span><span class="value">Bitcoin App</span></p>
  ...
  private calculateQualityScore(awardAmount: number): number {
    // Simple quality scoring based on $BAPP award
