...
export class GitHubAuthService {
  ...
  async getIssue(issueNumber: number): Promise<any> {
    const response = await fetch(
      `https://api.github.com/repos/bitcoin-apps-suite/bitcoin-app/issues/${issueNumber}`,
      {
        headers: {
          Accept: 'application/json',
          ...(this.accessToken && {
  ...
  async assignIssue(issueNumber: number): Promise<boolean> {
    ...
    const response = await fetch(
      `https://api.github.com/repos/bitcoin-apps-suite/bitcoin-app/issues/${issueNumber}/assignees`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
